...
export const InitButton = ({ label, behavior }) => {
    ...
    return (
        <Button onClick={actions.onSubmitInit} data-test-id="@testpage/init">
          Init
        </Button>
    );
...
<InitButton />
<Button onClick={() => subsequentCalls()} data-test-id="@testpage/subsequentCalls">
    Subsequent calls
</Button>
<Button
    onClick={() => subsequentCallsWithError()}
    data-test-id="@testpage/subsequentCallsWithError"
>

...
test('when user cancels permissions in popup it closes automatically', async ({
    ...
    await explorerPage.click("[data-test-id='@api-playground/collapsible-box']");
    await explorerPage.waitForSelector("button[data-test-id='@submit-button']", {
        state: 'visible',
    });
...
test('popup should be focused when a call is in progress and user triggers new call', async ({
    ...
    await explorerPage.waitForSelector(`[data-test-id='@submit-button']`, { state: 'visible' });
    await explorerPage.click(`[data-test-id='@submit-button']`, {

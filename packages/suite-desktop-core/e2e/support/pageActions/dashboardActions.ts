...
class DashboardActions {
    ...
    async ejectWallet(window: Page, walletIndex: number = 0) {
        const wallet = window.locator(
            `[data-test-id="@switch-device/wallet-on-index/${walletIndex}"]`,
        );
        await window
            .locator('[data-test-id="@switch-device/wallet-on-index/0/eject-button"]')
            .click();
        await window.locator('[data-test-id="@switch-device/eject"]').click();

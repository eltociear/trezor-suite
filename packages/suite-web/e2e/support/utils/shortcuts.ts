...
export const changeViewOnlyState = (walletIndex: number, desiredState: 'enabled' | 'disabled') => {
    ...
    cy.getTestElement(`@switch-device/wallet-on-index/${walletIndex}`).then(walletContainer => {
        ...
        if (!walletContainer.find(`[data-test-id="@viewOnlyStatus/${desiredState}"]`).length) {
            // if it is, open view-only settings container and change the state
            cy.wrap(walletContainer)
                .find('[data-test-id="@collapsible-box/icon-collapsed"]')
                .click();
            cy.wrap(walletContainer)
                .find('[data-test-id="@collapsible-box/body"]')
                .should('be.visible');
            cy.wrap(walletContainer).find(`[data-test-id$="/${desiredState}"]`).click();
            // close it to match the initial state
            cy.wrap(walletContainer)
                .find('[data-test-id="@collapsible-box/icon-expanded"]')
                .click();
        }
    });
};

...
const coinmarketMiddleware =
    ...
    (action: Action): Action => {
        if (action.type === COINMARKET_COMMON.LOAD_DATA) {
            ...
            const { coins } = api.getState().wallet.coinmarket.info;
            ...
            if (
                ...
                if (isDifferentAccount || !coins) {
                    loadPromises.push(
                        invityAPI.getCoins().then(coins => {
                            api.dispatch(coinmarketInfoAction.saveCoins(coins));
                        }),
                    );
                }
